...
<style>
:root{
  --bg:#07080c; --bg-2:#0b0d12; --surface:rgba(18,21,29,.78); --surface-solid:#11141b;
  --surface-2:rgba(25,29,39,.88); --border:rgba(255,255,255,.09); --border-strong:rgba(255,255,255,.15);
  --text:#f6f7fb; --text-dim:#9ba2b1; --muted:#697182;
  --accent:#f6d365; --accent-2:#ff9f68; --accent-3:#9a8cff;
  --accent-grad:linear-gradient(135deg,#f8dd75 0%,#ffad72 52%,#d99cff 100%);
  --accent-grad-2:linear-gradient(110deg,#8f83ff,#f8d86b,#ff9f68,#9b8cff);
  --danger:#ff6b7a; --success:#7ee2a8;
...
body{
  margin:0;color:var(--text);background:
    radial-gradient(circle at 15% 0%,rgba(154,140,255,.10),transparent 32%),
    radial-gradient(circle at 90% 100%,rgba(255,159,104,.08),transparent 34%),var(--bg);
...
button,textarea,input{font:inherit;color:inherit} button{cursor:pointer} a{color:inherit}
.bg-blobs{position:fixed;inset:0;overflow:hidden;z-index:0;pointer-events:none}
.grain{position:fixed;inset:0;z-index:2;pointer-events:none;opacity:.022;mix-blend-mode:screen;
...
.blob-1{width:520px;height:520px;background:radial-gradient(circle,#776cff,transparent 68%);top:-210px;left:-160px;animation:float1 26s ease-in-out infinite}
.blob-2{width:480px;height:480px;background:radial-gradient(circle,#ff9f68,transparent 70%);bottom:-190px;right:-130px;animation:float2 30s ease-in-out infinite}
.blob-3{width:380px;height:380px;background:radial-gradient(circle,#f6d365,transparent 70%);top:40%;left:58%;opacity:.13;animation:float3 34s ease-in-out infinite}
.blob-4{display:none}
@keyframes float1{0%,100%{transform:translate(0,0)}50%{transform:translate(70px,80px) scale(1.08)}}
@keyframes float2{0%,100%{transform:translate(0,0)}50%{transform:translate(-70px,-55px) scale(1.06)}}
@keyframes float3{0%,100%{transform:translate(0,0)}50%{transform:translate(-40px,55px) scale(1.12)}}
.app{position:relative;z-index:1;height:100vh;display:flex}
.sidebar{
  position:fixed;inset:0 auto 0 0;width:304px;max-width:86vw;padding:16px;
  background:rgba(9,11,16,.82);border-right:1px solid var(--border);
...
.sidebar-overlay{position:fixed;inset:0;background:rgba(0,0,0,.58);backdrop-filter:blur(4px);
  opacity:0;pointer-events:none;transition:opacity .28s;z-index:35}
.sidebar-overlay.open{opacity:1;pointer-events:auto}
.new-chat-btn{
  display:flex;align-items:center;justify-content:center;gap:9px;width:100%;padding:13px 15px;
  background:var(--accent-grad);color:#17150f;border:0;border-radius:15px;font-weight:850;
  box-shadow:0 12px 34px rgba(248,211,101,.12);transition:transform .18s,box-shadow .18s,filter .18s;
}
.new-chat-btn:hover{transform:translateY(-1px);filter:saturate(1.08);box-shadow:0 16px 42px rgba(248,211,101,.20)}
.new-chat-btn:active{transform:scale(.985)}
.chat-list{flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:5px;margin:14px -3px 0;padding:2px 3px}
.chat-item{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:11px 12px;
  border:1px solid transparent;border-radius:13px;color:var(--text-dim);cursor:pointer;
  transition:background .18s,color .18s,border-color .18s,transform .18s}
.chat-item:hover{background:rgba(255,255,255,.045);color:var(--text);transform:translateX(2px)}
.chat-item.active{background:linear-gradient(135deg,rgba(255,255,255,.075),rgba(255,255,255,.035));
  color:var(--text);border-color:var(--border)}
.chat-item .title{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13.5px}
.chat-item .del{opacity:0;background:none;border:0;color:var(--muted);padding:5px;border-radius:8px;transition:.15s}
.chat-item:hover .del{opacity:1}.chat-item .del:hover{color:var(--danger);background:rgba(255,107,122,.10)}
.chat-empty{color:var(--muted);font-size:12.5px;padding:16px 8px;text-align:center}
.sidebar-footer{border-top:1px solid var(--border);padding-top:12px;margin-top:10px;display:flex;flex-direction:column;gap:5px}
.sidebar-link{background:transparent;border:1px solid transparent;text-align:left;color:var(--text-dim);
  font-size:12.5px;padding:9px 10px;border-radius:10px;transition:.16s}
.sidebar-link:hover{background:rgba(255,255,255,.045);color:var(--text);border-color:var(--border)}
.build-tag{text-align:center;color:#596070;font-size:9.5px;letter-spacing:.55px;margin-top:5px;opacity:.75}
.main{flex:1;display:flex;flex-direction:column;height:100vh;min-width:0}
.topbar{
  height:68px;flex:none;display:flex;align-items:center;justify-content:space-between;padding:0 18px;
  border-bottom:1px solid var(--border);background:rgba(7,9,13,.62);backdrop-filter:blur(24px) saturate(150%);
  -webkit-backdrop-filter:blur(24px) saturate(150%);gap:12px;
}
.topbar-left{display:flex;align-items:center;gap:11px;min-width:0}
.icon-btn{
  width:40px;height:40px;border-radius:13px;border:1px solid var(--border);
...
.icon-btn:hover{background:rgba(255,255,255,.07);transform:translateY(-1px);border-color:var(--border-strong);box-shadow:0 8px 25px rgba(0,0,0,.16)}
.brand{display:flex;align-items:center;gap:8px;font-weight:900;font-size:17px;letter-spacing:.35px}
.brand .dot{width:8px;height:8px;border-radius:50%;background:var(--accent-grad);box-shadow:0 0 18px rgba(248,211,101,.75);animation:pulseDot 2.6s ease-in-out infinite}
@keyframes pulseDot{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.3);opacity:.65}}
.brand .grad-text{background:var(--accent-grad-2);background-size:300% 300%;-webkit-background-clip:text;background-clip:text;color:transparent;animation:gradShift 8s ease infinite}
@keyframes gradShift{0%{background-position:0 50%}50%{background-position:100% 50%}100%{background-position:0 50%}}
.brand .sub{opacity:.42;font-weight:700;color:var(--text-dim)}
.topbar-right{display:flex;align-items:center;gap:8px}
.pill{display:flex;align-items:center;gap:6px;border:1px solid var(--border);background:rgba(255,255,255,.035);
  border-radius:999px;padding:8px 12px;font-size:12px;color:var(--text-dim);white-space:nowrap;transition:.18s}
.pill.clickable:hover{background:rgba(255,255,255,.07);color:var(--text);transform:translateY(-1px);border-color:var(--border-strong)}
.pill.warn{color:var(--accent-2);border-color:rgba(255,159,104,.35);animation:warnPulse 2.2s ease-in-out infinite}
@keyframes warnPulse{0%,100%{box-shadow:0 0 0 rgba(255,159,104,0)}50%{box-shadow:0 0 0 4px rgba(255,159,104,.10)}}
.messages{flex:1;overflow-y:auto;padding:34px 0 12px;scroll-behavior:smooth}
.messages-inner{max-width:780px;margin:0 auto;padding:0 20px}
.message{display:flex;margin-bottom:20px;animation:msgIn .42s cubic-bezier(.22,1,.36,1)}
...
.avatar{width:32px;height:32px;border-radius:11px;flex:none;margin-right:10px;background:var(--accent-grad);
  display:flex;align-items:center;justify-content:center;font-size:14px;box-shadow:0 5px 20px rgba(248,211,101,.20)}
.bubble{max-width:min(650px,84%);padding:14px 17px;border-radius:19px;line-height:1.68;white-space:pre-wrap;font-size:15px}
.message.ai .bubble{background:rgba(18,21,29,.76);border:1px solid var(--border);border-top-left-radius:6px;box-shadow:0 10px 32px rgba(0,0,0,.18)}
.message.user .bubble{background:var(--accent-grad);color:#18150e;font-weight:650;border-top-right-radius:6px;box-shadow:0 10px 30px rgba(248,211,101,.13)}
.sources{max-width:650px;margin:-8px 0 22px 42px}.sources-title{color:var(--muted);font-size:12px;margin-bottom:7px}
.source-card{background:rgba(18,21,29,.70);border:1px solid var(--border);border-radius:12px;padding:10px 12px;margin-top:6px;transition:.18s}
.source-card:hover{border-color:var(--border-strong);transform:translateX(2px)}.source-card .s-title{font-size:13px;margin-bottom:3px}
.source-card a{font-size:12px;text-decoration:none;word-break:break-word;opacity:.78}
.typing-row{display:flex;align-items:center;margin-bottom:18px;animation:msgIn .35s cubic-bezier(.22,1,.36,1)}
.typing-bubble{position:relative;display:flex;align-items:center;gap:7px;background:rgba(18,21,29,.84);
  border:1px solid var(--border);border-radius:18px;border-top-left-radius:6px;padding:14px 18px;overflow:hidden}
.typing-bubble:after{content:"";position:absolute;top:0;bottom:0;width:80px;left:-90px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.10),transparent);transform:skewX(-18deg);animation:typingSweep 1.8s ease-in-out infinite}
@keyframes typingSweep{0%{left:-100px}60%,100%{left:130%}}
...
.typing-bubble span:nth-child(2){animation-delay:.15s;background:var(--accent-2)}.typing-bubble span:nth-child(3){animation-delay:.3s;background:var(--accent-3)}
@keyframes bounce{0%,60%,100%{transform:translateY(0);opacity:.42}30%{transform:translateY(-6px);opacity:1}}
.typing-label{font-size:12px;color:var(--muted);margin-left:9px}
.composer{padding:12px 18px 22px;flex:none}
.composer-inner{max-width:780px;margin:0 auto}
.composer-box{display:flex;align-items:flex-end;gap:10px;background:rgba(17,20,27,.82);border:1px solid var(--border);
  padding:8px 8px 8px 17px;border-radius:21px;backdrop-filter:blur(18px);transition:border-color .2s,box-shadow .2s,transform .2s}
.composer-box:focus-within{border-color:rgba(248,211,101,.35);box-shadow:0 0 0 4px rgba(248,211,101,.06),0 16px 50px rgba(0,0,0,.20)}
.composer textarea{flex:1;border:0;outline:0;background:transparent;resize:none;padding:10px 0;min-height:24px;max-height:150px;font-size:15px}
.composer textarea::placeholder{color:#6f7787}
.send{width:44px;height:44px;border-radius:14px;border:0;background:var(--accent-grad);display:flex;align-items:center;justify-content:center;
  flex:none;transition:transform .18s,opacity .18s,filter .18s,box-shadow .18s}
.send:hover{transform:scale(1.05);filter:saturate(1.08);box-shadow:0 8px 25px rgba(248,211,101,.18)}
.send:active{transform:scale(.94)}.send:disabled{opacity:.4;cursor:not-allowed;transform:none;box-shadow:none}
.hint{text-align:center;color:#565e6d;font-size:11px;margin-top:9px}.hint button{background:none;border:0;color:#737b8a;text-decoration:underline;font-size:11px;padding:0}
.modal-overlay{position:fixed;inset:0;background:rgba(4,5,8,.74);backdrop-filter:blur(10px) saturate(120%);
  display:flex;align-items:center;justify-content:center;z-index:100;opacity:0;pointer-events:none;transition:opacity .28s;padding:20px}
.modal-overlay.open{opacity:1;pointer-events:auto}
.modal-card{background:linear-gradient(180deg,rgba(20,24,33,.97),rgba(13,16,22,.98));border:1px solid var(--border-strong);
  border-radius:var(--radius-xl);max-width:480px;width:100%;padding:28px;max-height:88vh;overflow-y:auto;position:relative;
  transform:translateY(16px) scale(.97);transition:transform .3s cubic-bezier(.22,1,.36,1);box-shadow:var(--shadow)}
.modal-overlay.open .modal-card{transform:none}.eyebrow{display:inline-flex;padding:5px 9px;border-radius:999px;background:rgba(248,211,101,.08);border:1px solid rgba(248,211,101,.18);color:var(--accent);font-size:10px;font-weight:800;letter-spacing:.6px;margin-bottom:10px}.modal-card h2{margin:0 0 7px;font-size:22px;letter-spacing:-.3px}
.modal-card p{color:var(--text-dim);font-size:14px;line-height:1.65}
.modal-links{display:flex;flex-direction:column;gap:9px;margin:18px 0}
.modal-link-btn{display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,.045);
  border:1px solid var(--border);border-radius:13px;padding:13px 15px;color:var(--text);text-decoration:none;font-size:13.5px;transition:.18s}
.modal-link-btn:hover{background:rgba(255,255,255,.075);border-color:rgba(248,211,101,.35);transform:translateX(2px)}
.modal-checkbox-row{display:flex;align-items:flex-start;gap:10px;margin:16px 0;font-size:12.5px;color:var(--text-dim);line-height:1.5}
.modal-checkbox-row input{margin-top:3px;accent-color:var(--accent)}
.primary-btn{width:100%;background:var(--accent-grad);color:#17140f;border:0;border-radius:14px;padding:14px;font-weight:850;font-size:14.5px;
  transition:opacity .18s,transform .18s,filter .18s,box-shadow .18s}
.primary-btn:disabled{opacity:.4;cursor:not-allowed}.primary-btn:not(:disabled):hover{transform:translateY(-1px);filter:saturate(1.06);box-shadow:0 10px 28px rgba(248,211,101,.15)}
.ghost-btn{width:100%;background:transparent;color:var(--text-dim);border:1px solid var(--border);border-radius:13px;padding:12px;font-size:13.5px;margin-top:8px;transition:.16s}
.ghost-btn:hover{color:var(--text);border-color:var(--border-strong);background:rgba(255,255,255,.025)}
.close-x{position:absolute;top:14px;right:14px;width:34px;height:34px;border-radius:10px;background:rgba(255,255,255,.04);
  border:1px solid transparent;color:var(--text-dim);font-size:18px;transition:.16s}
.close-x:hover{color:var(--text);background:rgba(255,255,255,.08);border-color:var(--border)}
.plans{display:flex;flex-direction:column;gap:10px;margin:18px 0}.plan-card{border:1px solid var(--border);border-radius:16px;padding:15px 17px;
  background:rgba(255,255,255,.035);position:relative;transition:.18s}.plan-card:hover{transform:translateY(-2px);border-color:var(--border-strong)}
.plan-card.popular{border-color:rgba(248,211,101,.52);box-shadow:0 0 0 1px rgba(248,211,101,.12),0 12px 32px rgba(248,211,101,.06)}
.plan-badge{position:absolute;top:-10px;right:15px;background:var(--accent-grad);color:#17140f;font-size:10px;font-weight:850;padding:4px 10px;border-radius:999px}
.plan-top{display:flex;justify-content:space-between;align-items:baseline}.plan-name{font-weight:800;font-size:15px}.plan-price{font-weight:900;font-size:19px}.plan-meta{color:var(--text-dim);font-size:12px;margin-top:4px}
.free-banner{display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(248,211,101,.11),rgba(154,140,255,.07));
  border:1px solid rgba(248,211,101,.25);border-radius:14px;padding:12px 14px;margin-bottom:16px;font-size:12.5px;color:var(--text)}
.legal-mini{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.legal-mini a{color:var(--muted);font-size:10.5px;text-decoration:none}.legal-mini a:hover{color:var(--text)}
::-webkit-scrollbar{width:7px;height:7px}::-webkit-scrollbar-thumb{background:#2a3040;border-radius:8px}::-webkit-scrollbar-track{background:transparent}
@media(max-width:720px){
  .topbar{height:62px;padding:0 12px}.brand{font-size:16px}.pill{padding:7px 10px;font-size:11px}
  .messages{padding-top:22px}.messages-inner{padding:0 14px}.bubble{max-width:90%;font-size:14.5px}
  .composer{padding:10px 12px 16px}.composer-box{border-radius:18px}.sidebar{width:286px}
  .modal-card{padding:24px 20px;border-radius:22px}.sources{margin-left:8px}
}
...
<div class="app">
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <div class="sidebar-footer">
      <button class="sidebar-link" onclick="openInfoModal()">◌  Документы и поддержка</button>
      <button class="sidebar-link" onclick="openPricingModal()">✦  Тарифы</button>
      <div class="legal-mini">
        <a href="__PRIVACY_URL__" target="_blank" rel="noopener noreferrer">Конфиденциальность</a>
        <a href="__TERMS_URL__" target="_blank" rel="noopener noreferrer">Соглашение</a>
        <a href="__SUPPORT_URL__" target="_blank" rel="noopener noreferrer">Поддержка</a>
      </div>
      <div class="build-tag">ASCEND AI · mekbuda</div>
    </div>
  </aside>
  ...
  <div class="main">
    <div class="topbar">
      <div class="topbar-left">
        <button class="icon-btn" onclick="openSidebar()" aria-label="Открыть чаты">
      ...
      <div class="topbar-right">
        ...
        <button class="icon-btn" onclick="openInfoModal()" aria-label="Документы и поддержка" title="Документы и поддержка">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none">
            <path d="M7 3.8h7.2L19 8.6V20H7a2 2 0 0 1-2-2V5.8a2 2 0 0 1 2-2Z" stroke="currentColor" stroke-width="1.7"/>
            <path d="M14 3.8V9h5M9 12h6M9 15.5h6" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
          </svg>
        </button>
      </div>
    </div>
...
<div id="welcomeModal" class="modal-overlay">
  <div class="modal-card">
    <div class="eyebrow">ASCEND AI · mekbuda</div>
    <h2>Добро пожаловать 👋</h2>
    <p>Умный помощник для вопросов об уходе за собой, внешности, питании, сне и тренировках. Перед началом ознакомься с документами и условиями сервиса.</p>
...
<div id="infoModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <p>Здесь всегда доступны документы сервиса, поддержка и тарифы. Никаких скрытых разделов.</p>
    <div class="modal-links">
      <a class="modal-link-btn" href="__PRIVACY_URL__" target="_blank" rel="noopener noreferrer">Политика конфиденциальности <span>↗</span></a>
      <a class="modal-link-btn" href="__TERMS_URL__" target="_blank" rel="noopener noreferrer">Пользовательское соглашение <span>↗</span></a>
...
<div id="pricingModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <h2>Тарифы</h2>
    <div class="free-banner">🎁 Первый запрос бесплатный. Дальше — пакеты запросов без автосписаний.</div>
