...
# Пакеты запросов. Себестоимость одного запроса к платным LLM обычно
# составляет ~2-5 копеек, поэтому цены ниже дают комфортную маржу и при
# этом не выглядят как "по рублю за сообщение".
PRICING_PLANS = [
    {
        "id": "start",
        "name": "Старт",
...
# Фолбэк-хранилище в памяти процесса — используется только если
# Supabase не настроен. Сбрасывается при рестарте сервиса, поэтому
# для продакшена настоятельно рекомендуется настроить Supabase
# (таблицы описаны рядом с соответствующими функциями ниже).
...
def fetch_page_text(url):
    ...
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; ASCEND-AI/2.2)",
        "Accept-Language": "ru-RU,ru;q=0.9,en;q=0.8",
...
def get_memory(session_id):
    """Последние сообщения — используются как контекст для LLM."""
    if not SUPABASE_URL or not SUPABASE_SECRET_KEY:
        return []
...
def get_chat_messages(session_id, limit=MAX_CHAT_HISTORY):
    """Полная история сообщений чата — для отображения в интерфейсе."""
    if not SUPABASE_URL or not SUPABASE_SECRET_KEY:
        return []
...
# ============================================================
# CHAT SESSIONS (вкладка "чаты" — список диалогов пользователя)
# ============================================================
#
# Требуется таблица в Supabase (создать один раз вручную, в SQL editor):
#
...
#
# Без Supabase список чатов работать не будет (функции ниже просто
# вернут пустые результаты) — сам чат при этом продолжит отвечать,
# только без сохранения истории между визитами.
#
...
# ============================================================
#
# Каждому устройству/IP положен один бесплатный запрос. Дальше —
# только если на балансе есть купленные запросы (пополняется через
# админку, пока не подключена оплата по СБП). Ключ привязан к IP
# (а не к localStorage), поэтому очистка данных браузера или
# повторное открытие сайта не даёт новый бесплатный запрос.
#
# Требуется таблица в Supabase (создать один раз вручную, в SQL editor):
#
...
#   );
#
# Без Supabase используется хранилище в памяти процесса — работает,
# но сбрасывается при каждом рестарте сервиса.
#
...
def consume_access(key):
    """
    Проверяет и "тратит" доступ к одному запросу.
    Возвращает dict: {"allowed": bool, "mode": "free"/"paid"/"blocked", "credits": int}
    """
...
# ============================================================
#
# Кнопка "Админка" убрана из публичного интерфейса намеренно — панель
# управления теперь доступна только по прямому адресу /admin и всё
# равно защищена паролем (X-Admin-Token), но не выставлена напоказ
...
:root{
  ...
  --accent:#f7d45b;
  --accent-2:#ff9f5b;
  --accent-grad:linear-gradient(135deg,#f7d45b,#ff9f5b);
  --danger:#ff6b6b;
  --radius-lg:22px;
  --radius-md:16px;
  --radius-sm:10px;
...
.blob-1{width:520px;height:520px;background:radial-gradient(circle,#f7d45b,transparent 70%);top:-160px;left:-120px;animation:float1 22s ease-in-out infinite;}
.blob-2{width:460px;height:460px;background:radial-gradient(circle,#ff9f5b,transparent 70%);bottom:-140px;right:-100px;animation:float2 26s ease-in-out infinite;}
.blob-3{width:360px;height:360px;background:radial-gradient(circle,#7c6bff,transparent 70%);top:40%;left:60%;animation:float3 30s ease-in-out infinite;opacity:.22;}
...
.sidebar{
  ...
  background:rgba(14,14,19,.92);backdrop-filter:blur(18px);
...
.new-chat-btn{
  ...
  background:var(--accent-grad);color:#161616;font-weight:800;
  border:0;border-radius:var(--radius-md);padding:13px;margin-bottom:16px;
  transition:transform .15s, box-shadow .15s;
}
.new-chat-btn:hover{transform:translateY(-1px);box-shadow:0 8px 24px rgba(247,212,91,.25);}
...
.sidebar-link{background:none;border:0;text-align:left;color:var(--text-dim);font-size:13px;padding:8px 6px;border-radius:8px;}
.sidebar-link:hover{background:var(--surface);color:var(--text);}
.build-tag{text-align:center;color:#4b4b56;font-size:10px;letter-spacing:.4px;margin-top:6px;opacity:.7;}
...
.topbar{
  ...
  background:rgba(9,9,13,.7);backdrop-filter:blur(14px);gap:10px;
}
.icon-btn{
...
.brand .dot{width:8px;height:8px;border-radius:50%;background:var(--accent-grad);box-shadow:0 0 12px var(--accent);}
.brand span{opacity:.45;font-weight:700;}
...
.pill.warn{color:var(--accent-2);border-color:rgba(255,159,91,.35);}
.pill.clickable{cursor:pointer;}
.pill.clickable:hover{background:var(--surface-2);color:var(--text);}

/* ---------------- messages ---------------- */
.messages{flex:1;overflow-y:auto;padding:28px 0 10px;}
.messages-inner{max-width:760px;margin:0 auto;padding:0 18px;}
.message{display:flex;margin-bottom:18px;animation:msgIn .35s cubic-bezier(.25,.9,.35,1);}
...
.avatar{
  ...
  font-size:15px;box-shadow:0 4px 14px rgba(247,212,91,.25);
}
.bubble{
  max-width:min(640px,82%);padding:15px 18px;border-radius:18px;line-height:1.65;
  white-space:pre-wrap;font-size:15px;
}
.message.ai .bubble{background:var(--surface);border:1px solid var(--border);border-top-left-radius:6px;}
.message.user .bubble{background:var(--accent-grad);color:#171410;font-weight:600;border-top-right-radius:6px;}
.sources{max-width:640px;margin:-8px 0 22px 42px;}
.sources-title{color:var(--text-dim);font-size:12.5px;margin-bottom:7px;}
.source-card{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:10px 12px;margin-top:6px;transition:border-color .15s;}
.source-card:hover{border-color:#3a3a48;}
...
.typing-bubble{
  ...
  border:1px solid var(--border);border-radius:18px;border-top-left-radius:6px;
  padding:15px 20px;
}
.typing-bubble span{width:7px;height:7px;border-radius:50%;background:var(--accent);animation:bounce 1.1s infinite;}
...
.composer-box:focus-within{border-color:#3f3f52;box-shadow:0 0 0 4px rgba(247,212,91,.08);}
.composer textarea{
...
.send{
  width:44px;height:44px;border-radius:14px;border:0;background:var(--accent-grad);
...
/* ---------------- modals ---------------- */
.modal-overlay{
  position:fixed;inset:0;background:rgba(4,4,7,.72);backdrop-filter:blur(6px);
...
.modal-card{
  ...
  max-width:460px;width:100%;padding:28px;max-height:86vh;overflow-y:auto;
...
.modal-link-btn{
  ...
  padding:13px 16px;color:var(--text);text-decoration:none;font-size:14px;transition:border-color .15s;
}
.modal-link-btn:hover{border-color:var(--accent);}
...
.primary-btn{
  width:100%;background:var(--accent-grad);color:#161410;border:0;border-radius:13px;
  padding:14px;font-weight:800;font-size:15px;transition:opacity .15s, transform .15s;
}
...
.ghost-btn{
  ...
  border-radius:13px;padding:12px;font-size:14px;margin-top:8px;
}
.ghost-btn:hover{color:var(--text);border-color:#3a3a48;}
.close-x{position:absolute;top:16px;right:16px;background:none;border:0;color:var(--text-dim);font-size:20px;}
...
.plan-card.popular{border-color:var(--accent);box-shadow:0 0 0 1px rgba(247,212,91,.25);}
.plan-badge{
  ...
  <div class="main">
    <div class="topbar">
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <div class="brand"><span class="dot"></span>ASCEND <span>AI</span></div>
      </div>
...
<div id="pricingModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <h2>Тарифы</h2>
    <p>Первый запрос всегда бесплатный. Дальше — пакеты запросов, без автосписаний.</p>
...
# ============================================================
# HTML — АДМИН-ПАНЕЛЬ (доступна только по прямой ссылке /admin)
# ============================================================
...
.header { height: 70px; border-bottom: 1px solid #252529; display: flex; align-items: center; }
.header-inner { display: flex; justify-content: space-between; align-items: center; width:100%; }
.logo { font-size: 20px; font-weight: 900; letter-spacing: .5px; }
.logo span { opacity: .4; }
.back-link { color:#898991; text-decoration:none; font-size:13px; }
.admin { padding: 35px 0 60px; }
...
<div class="logo">ASCEND <span>AI · Админка</span></div>
...
<h2>⚙️ Вход</h2>
<p style="opacity:.7;">Панель управления нейросетью, знаниями, ключами и балансами.</p>
<div class="field">
<label>Пароль</label>
...
<div class="card">
<h2>💳 Пополнить баланс вручную (пока нет оплаты по СБП)</h2>
<p style="opacity:.7;font-size:13px;margin-top:-8px;">Баланс привязан к IP-адресу пользователя. Узнать его можно, например, попросив пользователя написать в поддержку.</p>
<div class="row2">
<div class="field"><label>IP-адрес пользователя</label><input id="topupIp" placeholder="1.2.3.4"></div>
<div class="field"><label>Количество запросов</label><input id="topupCredits" type="number" value="50"></div>
</div>
...
async function topUpCredits() {
    const ip = document.getElementById("topupIp").value.trim();
    const credits = parseInt(document.getElementById("topupCredits").value, 10) || 0;
    const status = document.getElementById("topupStatus");
    if (!ip) { status.textContent = "Укажи IP-адрес."; return; }
    status.textContent = "Начисляю...";
    try {
        const response = await fetch("/api/admin/credits", {
            method: "POST",
            headers: adminHeaders(),
            body: JSON.stringify({ ip, credits })
        });
        let data;
        try { data = await response.json(); } catch { data = { detail: "Ошибка." }; }
        if (!response.ok) { status.textContent = data.detail || "Ошибка."; return; }
        status.textContent = `✅ Начислено. Текущий баланс: ${data.credits}`;
    } catch (e) {
        status.textContent = "Ошибка соединения.";
    }
}
...
# ============================================================
# SMALL TALK / GREETINGS (обрабатываются БЕЗ похода в поиск, LLM и без
# списания баланса)
# ============================================================
...
def chat(data: ChatRequest, request: Request):
    ...
    if small_talk_answer:
        print("SMALL TALK DETECTED — skipping search/LLM/billing")
...
# ============================================================
# ADMIN CREDITS TOP-UP (временно, до подключения оплаты по СБП)
# ============================================================

@app.post("/api/admin/credits")
def admin_topup_credits(request: Request, data: CreditTopUp):
    check_admin(request)
...
async def startup():
    ...
    if ADMIN_PASSWORD == "CHANGE_THIS_PASSWORD":
        ...
        print("Админ-панель теперь находится по адресу /admin (без ссылки", flush=True)
        print("на главной странице).", flush=True)
        print("!" * 60, flush=True)
