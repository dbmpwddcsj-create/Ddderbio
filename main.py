...
# ============================================================
# НОВЫЙ HTML — ПОЛНОСТЬЮ ПЕРЕРАБОТАННЫЙ ФРОНТЕНД
# ============================================================
...
/* Анимированный фон с частицами (упрощённый вариант) */
.bg {
  position: fixed; inset:0; z-index:0;
  background: radial-gradient(circle at 20% 30%, rgba(247,212,91,0.06), transparent 60%),
              radial-gradient(circle at 80% 70%, rgba(139,123,255,0.08), transparent 60%);
}
.bg::after {
  content: '';
  position: absolute; inset:0;
  background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 400 400' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='2' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)' opacity='0.04'/%3E%3C/svg%3E");
  pointer-events: none;
}
...
.sidebar-overlay {
  ...
  opacity:0; pointer-events:none; transition: opacity 0.3s; z-index:35;
}
.sidebar-overlay.open { opacity:1; pointer-events:auto; }

.new-chat-btn {
  display: flex; align-items:center; gap:10px; justify-content:center;
  background: linear-gradient(135deg, #f7d45b 0%, #ff9f5b 55%, #ff6bd4 100%);
...
.new-chat-btn:hover { transform: translateY(-2px); box-shadow: 0 12px 32px rgba(247,212,91,0.25); background-position: 100% 50%; }
...
.chat-item:hover { background: rgba(255,255,255,0.04); color: #f0f0f5; transform: translateX(3px); }
.chat-item.active { background: rgba(255,255,255,0.06); color: #f0f0f5; border-color: rgba(255,255,255,0.08); }
.chat-item .title { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 14px; flex:1; }
.chat-item .del {
  opacity: 0; transition: opacity 0.15s;
  background: none; border:0; color: #6b6b78; padding:4px; border-radius:8px; cursor:pointer;
}
.chat-item:hover .del { opacity:1; }
.chat-item .del:hover { color: #ff6b6b; background: rgba(255,107,107,0.1); }
...
.sidebar-link {
  ...
  color: #93939f; font-size: 13px;
  padding: 8px 6px; border-radius: 8px;
  transition: background 0.15s, color 0.15s;
  cursor: pointer;
  text-decoration: none;
}
.sidebar-link:hover { background: rgba(255,255,255,0.04); color: #f0f0f5; }
.build-tag {
  text-align: center; color: #4b4b56; font-size: 10px; letter-spacing: 0.5px;
...
.icon-btn:hover { background: rgba(255,255,255,0.07); transform: translateY(-1px); border-color: rgba(255,255,255,0.12); }
.brand {
...
.brand .dot {
  ...
  border-radius: 50%;
  background: linear-gradient(135deg, #f7d45b, #ff9f5b);
  box-shadow: 0 0 16px rgba(247,212,91,0.4);
  animation: pulse-dot 2.4s ease-in-out infinite;
}
@keyframes pulse-dot { 0%,100%{ transform:scale(1); opacity:1; } 50%{ transform:scale(1.4); opacity:0.6; } }
.brand .grad-text {
  background: linear-gradient(120deg, #f7d45b, #ff9f5b, #ff6bd4, #8b7bff);
...
.brand .sub { opacity:0.4; font-weight:600; -webkit-text-fill-color: #b0b0bc; }
...
.pill.clickable:hover { background: rgba(255,255,255,0.06); color: #f0f0f5; transform: translateY(-1px); }
.pill.warn { color: #ff9f5b; border-color: rgba(255,159,91,0.3); animation: warn-pulse 2s ease-in-out infinite; }
@keyframes warn-pulse { 0%,100%{ box-shadow:0 0 0 0 rgba(255,159,91,0); } 50%{ box-shadow:0 0 0 5px rgba(255,159,91,0.12); } }

/* ----- MESSAGES AREA ----- */
.messages {
  flex:1; overflow-y: auto; padding: 24px 0 8px; scroll-behavior: smooth;
}
.messages-inner {
...
.message {
  ...
  animation: msg-in 0.5s cubic-bezier(.22,1,.36,1);
}
@keyframes msg-in { from { opacity:0; transform: translateY(18px) scale(0.96); } to { opacity:1; transform: translateY(0) scale(1); } }
.message.user { justify-content: flex-end; }
.message.ai { justify-content: flex-start; }

.avatar {
  width: 34px; height: 34px; border-radius: 10px; flex:none; margin-right: 12px;
  background: linear-gradient(135deg, #f7d45b, #ff9f5b);
  display: flex; align-items: center; justify-content: center;
  font-size: 16px;
  box-shadow: 0 4px 20px rgba(247,212,91,0.25);
}
.bubble {
...
.message.user .bubble {
  ...
  box-shadow: 0 6px 24px rgba(247,212,91,0.18);
}

/* Sources */
.sources {
  max-width: 640px; margin: -6px 0 24px 46px;
}
.sources-title { color: #93939f; font-size: 12px; margin-bottom: 6px; }
.source-card {
  background: rgba(255,255,255,0.03);
...
.source-card:hover { border-color: rgba(255,255,255,0.12); transform: translateX(4px); }
.source-card .s-title { font-size: 13px; margin-bottom: 2px; }
.source-card a { font-size: 12px; text-decoration: none; word-break: break-word; opacity:0.7; color: #f7d45b; }

/* ----- TYPING INDICATOR (новый дизайн) ----- */
.typing-row {
  display: flex; align-items: center; margin-bottom: 20px;
  animation: msg-in 0.4s cubic-bezier(.22,1,.36,1);
}
.typing-bubble {
...
.typing-bubble .dot:nth-child(2) { animation-delay: 0.2s; background: #ff9f5b; }
.typing-bubble .dot:nth-child(3) { animation-delay: 0.4s; background: #8b7bff; }
@keyframes bounce { 0%,60%,100% { transform: translateY(0); opacity:0.5; } 30% { transform: translateY(-6px); opacity:1; } }
.typing-label {
  font-size: 12.5px; color: #93939f; margin-left: 6px;
}

/* ----- COMPOSER ----- */
.composer {
  padding: 14px 18px 22px; flex:none;
}
...
.composer-box:focus-within {
  border-color: rgba(247,212,91,0.3);
  box-shadow: 0 0 0 4px rgba(247,212,91,0.06);
}
.composer textarea {
  flex:1; border:0; outline:0; background:transparent; resize:none;
  padding: 10px 0; min-height: 24px; max-height: 150px;
  font-size: 15px; color: #f0f0f5;
}
.composer textarea::placeholder { color: #6b6b78; }
.send {
...
.hint {
  text-align: center; color: #5b5b66; font-size: 11.5px; margin-top: 10px;
}
.hint button { background: none; border:0; color: #5b5b66; text-decoration: underline; font-size: 11.5px; padding:0; cursor:pointer; }
...
.modal-card {
  ...
  border: 1px solid rgba(255,255,255,0.06);
  border-radius: 28px;
  max-width: 500px; width:100%; padding: 32px 28px;
...
.modal-card p { color: #b0b0bc; font-size: 14.5px; line-height:1.6; }
.modal-links {
...
.modal-link-btn {
  ...
  border-radius: 14px;
  padding: 14px 18px;
  color: #f0f0f5; text-decoration: none; font-size: 14px;
  transition: border-color 0.2s, transform 0.15s;
}
.modal-link-btn:hover { border-color: rgba(247,212,91,0.3); transform: translateX(4px); }
.modal-checkbox-row {
  display: flex; align-items: flex-start; gap: 10px;
  margin: 16px 0; font-size: 13px; color: #b0b0bc;
}
.modal-checkbox-row input { margin-top: 3px; accent-color: #f7d45b; width:18px; height:18px; }
.primary-btn {
  width:100%; background: linear-gradient(135deg, #f7d45b, #ff9f5b);
  color: #171410; border:0; border-radius: 14px;
...
.ghost-btn {
  width:100%; background:transparent; color:#b0b0bc;
...
.ghost-btn:hover { color: #f0f0f5; border-color: rgba(255,255,255,0.12); }
.close-x {
  position: absolute; top:16px; right:16px;
  background:none; border:0; color:#93939f; font-size:20px;
  transition: color 0.2s, transform 0.2s;
  cursor:pointer;
}
.close-x:hover { color:#f0f0f5; transform: rotate(90deg); }
...
.plan-card:hover { transform: translateY(-2px); border-color: rgba(255,255,255,0.12); }
.plan-card.popular { border-color: rgba(247,212,91,0.3); box-shadow: 0 0 0 1px rgba(247,212,91,0.1); }
.plan-badge {
  position:absolute; top:-10px; right:16px;
  background: linear-gradient(135deg, #f7d45b, #ff9f5b);
  color: #171410; font-size:11px; font-weight:800;
...
.plan-meta { color:#93939f; font-size:12.5px; margin-top:4px; }
.free-banner {
  display:flex; align-items:center; gap:10px;
  background: linear-gradient(135deg, rgba(247,212,91,0.12), rgba(255,159,91,0.06));
  border:1px solid rgba(247,212,91,0.15);
...
<div class="app">
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <div id="chatList" class="chat-list">
      <div class="chat-empty" style="color:#6b6b78;font-size:13px;padding:14px 8px;text-align:center;">Загрузка чатов…</div>
    </div>
    <div class="sidebar-footer">
      <button class="sidebar-link" onclick="openInfoModal()">ℹ️ Инфо, поддержка, тарифы</button>
      <a class="sidebar-link" href="__PRIVACY_URL__" target="_blank" rel="noopener noreferrer">📄 Политика конфиденциальности</a>
      <a class="sidebar-link" href="__TERMS_URL__" target="_blank" rel="noopener noreferrer">📄 Пользовательское соглашение</a>
      <a class="sidebar-link" href="__SUPPORT_URL__" target="_blank" rel="noopener noreferrer">✉️ Поддержка Telegram</a>
      <div class="build-tag">mekbuda · v3.0</div>
    </div>
  </aside>
...
Первый запрос – бесплатно 🎁 Что тебя интересует?</div>
    ...
    <div class="composer">
      <div class="composer-inner">
        ...
        <div class="hint">Ответы носят справочный характер · <button onclick="openInfoModal()">Тарифы и поддержка</button></div>
      </div>
    </div>
  </div>
</div>
...
<div id="welcomeModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <p>ASCEND AI – ассистент по уходу за собой. Первый запрос – бесплатно, дальше – недорогие пакеты.</p>
    <div class="free-banner">🎁 Первый запрос всегда бесплатный. Выберите пакет:</div>
    <div id="welcomePlans" class="plans"></div>
...
<div id="pricingModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <h2>Тарифы</h2>
    <div class="free-banner">🎁 Первый запрос всегда бесплатный. Дальше – пакеты без автосписаний.</div>
    <div id="plansList" class="plans"><div class="chat-empty" style="color:#6b6b78;text-align:center;">Загрузка тарифов…</div></div>
    <a class="ghost-btn" href="__SUPPORT_URL__" target="_blank" rel="noopener noreferrer">Оплата картой / СБП – скоро. Написать в поддержку</a>
  </div>
</div>
...
<div id="paywallModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <p>Бесплатный запрос уже использован. Чтобы продолжить, выберите пакет запросов.</p>
...
const CHATS_STORAGE_KEY = "ascend_chats";
const MESSAGES_PREFIX = "ascend_messages_";
...
function getLocalChats() {
    try { return JSON.parse(localStorage.getItem(CHATS_STORAGE_KEY)) || []; } catch { return []; }
}
function saveLocalChats(chats) { localStorage.setItem(CHATS_STORAGE_KEY, JSON.stringify(chats)); }
function getLocalMessages(sessionId) {
    try { return JSON.parse(localStorage.getItem(MESSAGES_PREFIX + sessionId)) || []; } catch { return []; }
}
function saveLocalMessages(sessionId, messages) { localStorage.setItem(MESSAGES_PREFIX + sessionId, JSON.stringify(messages)); }
function addLocalMessage(sessionId, role, content) {
    ...
    if (chat) { chat.updated_at = new Date().toISOString(); saveLocalChats(chats); }
}
function deleteLocalChat(sessionId) {
    let chats = getLocalChats();
    chats = chats.filter(c => c.session_id !== sessionId);
...
function loadWelcomePlans(){
    fetch('/api/pricing')
        .then(r => r.json())
        .then(plans => {
            const container = document.getElementById('welcomePlans');
            container.innerHTML = '';
        ...
        .catch(() => { document.getElementById('welcomePlans').innerHTML = '<div class="chat-empty" style="color:#6b6b78;">Не удалось загрузить тарифы</div>'; });
}
...
async function loadChatList(){
    ...
    try {
        ...
        if (!Array.isArray(chats) || chats.length === 0) chats = getLocalChats();
        else saveLocalChats(chats);
        if (chats.length === 0) {
            list.innerHTML = '<div class="chat-empty" style="color:#6b6b78;">Пока нет сохранённых чатов</div>';
            return;
        }
        list.innerHTML = '';
        chats.forEach(chat => {
...
async function createNewChat(){
    try {
        ...
        const chats = getLocalChats();
        chats.unshift({ session_id: data.session_id, title: 'Новый чат', created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
        saveLocalChats(chats);
        renderWelcomeOnly();
        closeSidebar();
        loadChatList();
        ...
        currentChatId = sid;
        localStorage.setItem(CHAT_KEY, sid);
        const chats = getLocalChats();
        chats.unshift({ session_id: sid, title: 'Новый чат', created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
        saveLocalChats(chats);
        renderWelcomeOnly();
        closeSidebar();
        loadChatList();
...
async function deleteChat(event, sessionId){
    ...
    try { await fetch('/api/chats/' + sessionId, { method: 'DELETE' }); } catch(e) {}
    deleteLocalChat(sessionId);
    if (sessionId === currentChatId) await createNewChat();
    else loadChatList();
}
...
async function loadCurrentChatMessages(){
    ...
    try {
        ...
        if (Array.isArray(msgs) && msgs.length > 0) {
            ...
            saveLocalMessages(currentChatId, msgs);
            return;
        }
    } catch(e) {}
    const msgs = getLocalMessages(currentChatId);
    if (msgs.length === 0) renderWelcomeOnly();
    else {
        inner.innerHTML = '';
        msgs.forEach(m => addMessage(m.role === 'user' ? 'user' : 'ai', m.content));
    }
...
function addMessage(role, text){
    ...
    if (role === 'user') wrapper.innerHTML = `<div class="bubble"></div>`;
    else wrapper.innerHTML = `<div class="avatar">🧠</div><div class="bubble"></div>`;
...
function showTyping(){
    ...
    row.id = 'typingRow';
    row.innerHTML = `<div class="avatar">🧠</div><div class="typing-bubble"><span class="dot"></span><span class="dot"></span><span class="dot"></span><span class="typing-label">${TYPING_LABELS[0]}</span></div>`;
...
function hideTyping(){
    ...
    if (row) { clearInterval(row.dataset.timer); row.remove(); }
}
...
async function sendMessage(){
    ...
    addLocalMessage(currentChatId, 'user', message);
    input.value = '';
    autosize(input);
    ...
    try {
        ...
        if (response.status === 402) {
            addMessage('ai', data.detail || 'Бесплатный запрос использован. Пополните баланс.');
            ...
            if (data.answer) addLocalMessage(currentChatId, 'assistant', data.answer);
...
async function loadPricing(){
    ...
    try {
        ...
        box.innerHTML = '<div class="chat-empty" style="color:#6b6b78;">Не удалось загрузить тарифы</div>';
    }
}
...
(async function init(){
    ...
    loadBalance();
    if (!currentChatId) await createNewChat();
    else await loadCurrentChatMessages();
})();
</script>
</body>
</html>
