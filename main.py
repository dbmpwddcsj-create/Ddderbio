...
# ============================================================
# HTML — ГЛАВНАЯ СТРАНИЦА ЧАТА (обновлённый дизайн)
# ============================================================
...
.sidebar-link{
  background:none;border:0;text-align:left;color:var(--text-dim);font-size:13px;
  padding:8px 6px;border-radius:8px;transition:background .15s,color .15s;
}
...
.message{display:flex;margin-bottom:18px;animation:msgIn .5s cubic-bezier(.22,1,.36,1);}
...
.typing-row{display:flex;align-items:center;margin-bottom:18px;animation:msgIn .4s cubic-bezier(.22,1,.36,1);}
.typing-bubble{
  position:relative;display:flex;align-items:center;gap:8px;background:var(--surface);
  border:1px solid transparent;border-radius:18px;border-top-left-radius:6px;
  padding:15px 20px;overflow:hidden;min-width:80px;
}
...
.typing-bubble .dot{
  width:8px;height:8px;border-radius:50%;background:var(--accent);animation:bounce 1.1s infinite;position:relative;z-index:1;
}
.typing-bubble .dot:nth-child(2){animation-delay:.15s;background:var(--accent-2);}
.typing-bubble .dot:nth-child(3){animation-delay:.3s;background:var(--accent-3);}
...
.modal-card{
  ...
  max-width:480px;width:100%;padding:28px;max-height:86vh;overflow-y:auto;position:relative;
...
<div id="welcomeModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <div class="free-banner">🎁 Первый запрос всегда бесплатный. Дальше — выберите пакет:</div>
...
function loadWelcomePlans(){
    fetch("/api/pricing")
        ...
        .catch(() => {
            document.getElementById("welcomePlans").innerHTML = '<div class="chat-empty">Не удалось загрузить тарифы</div>';
        });
}
...
async function loadChatList(){
    ...
    try {
        ...
        if (!Array.isArray(chats) || chats.length === 0) {
            ...
            // merge/sync with local? we'll trust server if exists
            saveLocalChats(chats);
        }
        if (chats.length === 0) {
...
async function sendMessage(){
    ...
    try {
        ...
        if (response.status === 402) {
            ...
            if (data.answer) {
                addLocalMessage(currentChatId, "assistant", data.answer);
            }
...
# ============================================================
# MANUAL CREDIT TOP-UP (serviсe endpoint)
# ============================================================
