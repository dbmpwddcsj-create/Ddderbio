...
def chat_belongs_to_device(session_id, device_id):
...
def get_client_ip(request: Request):
    # Render/standard reverse proxies expose the original client IP here.
    # We never accept an IP supplied by the frontend itself.
...
def get_client_key(request: Request):
    # IP is deliberately the identity for the one-time free request:
    # clearing localStorage/cookies, reopening the site or changing tabs
    # cannot mint another free request.
    ip = get_client_ip(request)
    return hashlib.sha256(
        f"ascend-credits:v3:{ip}".encode("utf-8")
    ).hexdigest()[:40]
...
def consume_access(key):
    # Single-process race protection. For multiple Render instances,
    # make this operation atomic in Postgres (see deployment note below).
    with _credit_lock:
...
.typing-bubble::after{
  ...
  transform:skewX(-18deg);animation:typingSweep 1.8s ease-in-out infinite;
}
@keyframes typingSweep{0%{left:-35%;opacity:0}25%{opacity:1}75%{opacity:1}100%{left:115%;opacity:0}}
...
@media(max-width:720px){
  ...
  .sidebar{width:280px;}
  .topbar-right .pill{max-width:170px;overflow:hidden;text-overflow:ellipsis;}
}
...
<div id="pricingModal" class="modal-overlay">
  <div class="modal-card">
    ...
    <h2>Тарифы</h2>
    <div class="free-banner">🎁 Первый запрос бесплатный. Дальше — недорогие пакеты, без автосписаний.</div>
...
async function deleteChat(event, sessionId){
    ...
    try {
        await fetch("/api/chats/" + sessionId + "?device_id=" + encodeURIComponent(deviceId), { method: "DELETE" });
...
async function loadCurrentChatMessages(){
    const inner = document.getElementById("messagesInner");
    try {
        const res = await fetch("/api/chats/" + currentChatId + "/messages?device_id=" + encodeURIComponent(deviceId));
...
async function sendMessage(){
    ...
    try {
        const response = await fetch("/api/chat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ session_id: currentChatId, message: message, device_id: deviceId })
        });
...
def chat(data: ChatRequest, request: Request):
    ...
    if not data.device_id or len(data.device_id) > 120:
        raise HTTPException(400, "Некорректный идентификатор устройства.")

    if not chat_belongs_to_device(data.session_id, data.device_id):
        raise HTTPException(404, "Чат не найден.")

    # Consume access BEFORE writing anything to the chat. Greetings also count.
...
def chat_messages_endpoint(session_id: str, device_id: str = ""):
    if not chat_belongs_to_device(session_id, device_id):
        raise HTTPException(404, "Чат не найден.")
    return get_chat_messages(session_id)


@app.delete("/api/chats/{session_id}")
def delete_chat(session_id: str, device_id: str = ""):
    if not chat_belongs_to_device(session_id, device_id):
        raise HTTPException(404, "Чат не найден.")
...
async def startup():
    ...
    if SUPABASE_URL and SUPABASE_SECRET_KEY:
        print(
            "Проверьте таблицы chat_sessions, chat_messages, user_credits и training_log. "
            "Для нескольких инстансов приложения используйте атомарное списание кредитов "
            "на стороне Postgres.",
            flush=True,
        )
