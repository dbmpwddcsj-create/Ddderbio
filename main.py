...
/* -------- Ambient background: removed glow, ChatGPT uses flat black -------- */
.bg-glow { position: fixed; inset: 0; z-index: 0; overflow: hidden; pointer-events: none; opacity: 0; }
.blob { display: none; }
...
.sidebar-head { padding: 18px 20px 14px; display: flex; align-items: center; justify-content: space-between; }
...
.new-chat-btn {
    margin: 4px 12px 10px; padding: 12px 12px; border-radius: var(--radius-md);
...
.chat-list { flex: 1; overflow-y: auto; padding: 8px 12px 10px; }
.chat-item {
    display: flex; align-items: center; gap: 8px; padding: 11px 12px; border-radius: var(--radius-sm);
    margin-bottom: 1px; cursor: pointer; transition: background .15s; position: relative;
}
.chat-item:hover { background: var(--panel-hover); }
.chat-item.active { background: rgba(255,255,255,0.09); border: none; }
...
.chat-item .del {
    opacity: 0; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center;
    font-size: 14px; color: var(--text-faint); transition: opacity .15s, background .15s, color .15s; flex-shrink: 0;
}
.chat-item:hover .del { opacity: 1; }
.chat-item .del:hover { background: rgba(255,92,92,0.18); color: #ff8a8a; }
.sidebar-foot { padding: 10px 12px 16px; border-top: 1px solid var(--panel-border); }
.credits-pill {
    display: flex; align-items: center; justify-content: space-between; gap: 8px;
    background: transparent; border: none; border-radius: var(--radius-sm);
    padding: 10px 12px; font-size: 13px; margin-bottom: 4px; color: var(--text-dim);
}
.credits-pill b { color: var(--accent-a); font-weight: 600; }
.foot-links { display: flex; flex-wrap: wrap; gap: 6px; padding: 0 4px; }
.link-chip {
    font-size: 12.5px; padding: 8px 4px; border-radius: var(--radius-sm); border: none;
...
.overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 30; opacity: 0; pointer-events: none; transition: opacity .25s; }
...
.topbar {
    ...
    padding: 0 14px; border-bottom: none; background: var(--bg);
}
...
.pill-btn {
    padding: 8px 14px; border-radius: 30px; border: 1px solid var(--panel-border);
    background: transparent; font-size: 13px; font-weight: 500; display: flex; align-items: center; gap: 6px;
    transition: background .15s;
    color: var(--text-dim);
}
.pill-btn:hover { background: var(--panel-hover); }
.pill-btn.gradient { background: var(--accent-a); color: #fff; border: none; font-weight: 600; }

/* -------- Chat area -------- */
.chat-area { flex: 1; overflow-y: auto; padding: 28px 0 10px; }
.chat-inner { width: min(780px, 92%); margin: 0 auto; }
.message { display: flex; margin-bottom: 22px; opacity: 0; transform: translateY(6px); animation: rise .3s ease forwards; }
@keyframes rise { to { opacity: 1; transform: translateY(0); } }
.message.user { justify-content: flex-end; }
.message.ai { justify-content: flex-start; }
.bubble { max-width: min(680px, 88%); padding: 13px 17px; border-radius: 20px; line-height: 1.6; white-space: pre-wrap; font-size: 15px; }
.message.ai .bubble {
...
.message.user .bubble {
    background: #2f2f2f; color: var(--text); font-weight: 400;
    border-radius: 20px;
}
...
.avatar {
    ...
    background: var(--accent-a); font-size: 13px; margin-top: 2px;
}
.sources { width: min(680px, 88%); margin: -8px 0 20px 0; }
.source-card {
    background: rgba(255,255,255,0.04); border: 1px solid var(--panel-border); border-radius: var(--radius-sm);
    padding: 11px 13px; margin-top: 7px; font-size: 12.5px;
}
.source-card .stitle { opacity: .85; margin-bottom: 4px; }
.source-card a { color: var(--accent-a); text-decoration: none; word-break: break-word; }
.sources-heading { opacity: .5; font-size: 12px; margin: 0 0 6px 0; }
...
/* -------- Composer -------- */
.composer-wrap { padding: 10px 0 22px; flex-shrink: 0; }
.composer-inner { width: min(780px, 92%); margin: 0 auto; }
.composer-box {
    display: flex; align-items: flex-end; gap: 10px; background: #2f2f2f;
    border: none; padding: 12px 12px 12px 20px; border-radius: 30px;
    transition: box-shadow .2s;
}
.composer-box:focus-within { box-shadow: 0 0 0 2px rgba(74,141,255,0.35); }
...
.hint-row { text-align: center; font-size: 11px; color: var(--text-faint); margin-top: 10px; }
...
.modal-box {
    ...
    background: #171717; border: 1px solid var(--panel-border); border-radius: var(--radius-lg);
...
.doc-link {
    ...
    border-radius: var(--radius-sm); background: rgba(255,255,255,0.04); border: 1px solid var(--panel-border);
...
.btn-primary {
    flex: 1; padding: 13px; border-radius: var(--radius-sm); border: 0; background: var(--accent-a);
...
.plan-card {
    ...
    background: rgba(255,255,255,0.03); display: flex; align-items: center; justify-content: space-between; gap: 12px;
    transition: border-color .15s;
}
.plan-card.hl { border-color: var(--accent-a); background: rgba(74,141,255,0.09); }
...
.buy-btn {
    padding: 9px 14px; border-radius: 20px; border: 0; background: var(--accent-a);
...
.client-id-note code { background: rgba(255,255,255,0.06); padding: 2px 6px; border-radius: 6px; }

/* -------- Admin -------- */
.admin { display: none; }
.admin-wrap { width: min(900px, 94%); margin: 0 auto; padding: 30px 0 60px; overflow-y: auto; height: 100%; }
.card { background: rgba(255,255,255,0.03); border: 1px solid var(--panel-border); border-radius: var(--radius-lg); padding: 22px; margin-bottom: 18px; }
...
.primary { background: var(--accent-a); color: #fff; border: 0; border-radius: 11px; padding: 12px 17px; font-weight: 700; }
